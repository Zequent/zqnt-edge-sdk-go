// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        (unknown)
// source: mission-autonomy.proto

package proto

import (
	proto "github.com/Zequent/zqnt-edge-sdk-go/gen/common/base/proto"
	_ "github.com/Zequent/zqnt-edge-sdk-go/gen/common/proto"
	proto2 "github.com/Zequent/zqnt-edge-sdk-go/gen/execution/contracts/proto"
	proto3 "github.com/Zequent/zqnt-edge-sdk-go/gen/missionautonomy/contracts/proto"
	proto1 "github.com/Zequent/zqnt-edge-sdk-go/gen/missionautonomy/domain/types/proto"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type EvaluateDetectionRequest struct {
	state              protoimpl.MessageState     `protogen:"open.v1"`
	Base               *proto.RequestBase         `protobuf:"bytes,1,opt,name=base,proto3" json:"base,omitempty"`
	DetectionId        string                     `protobuf:"bytes,2,opt,name=detection_id,json=detectionId,proto3" json:"detection_id,omitempty"`
	AssetSn            string                     `protobuf:"bytes,3,opt,name=asset_sn,json=assetSn,proto3" json:"asset_sn,omitempty"`          // detecting asset SN
	ObjectType         string                     `protobuf:"bytes,4,opt,name=object_type,json=objectType,proto3" json:"object_type,omitempty"` // e.g. "DRONE", "PERSON", "VEHICLE"
	Confidence         float32                    `protobuf:"fixed32,5,opt,name=confidence,proto3" json:"confidence,omitempty"`
	DetectionLatitude  float64                    `protobuf:"fixed64,6,opt,name=detection_latitude,json=detectionLatitude,proto3" json:"detection_latitude,omitempty"`
	DetectionLongitude float64                    `protobuf:"fixed64,7,opt,name=detection_longitude,json=detectionLongitude,proto3" json:"detection_longitude,omitempty"`
	DetectionAltitude  float64                    `protobuf:"fixed64,8,opt,name=detection_altitude,json=detectionAltitude,proto3" json:"detection_altitude,omitempty"`
	OrganizationId     *string                    `protobuf:"bytes,10,opt,name=organization_id,json=organizationId,proto3,oneof" json:"organization_id,omitempty"`
	RuntimeConfig      *proto1.DynamicConfigProto `protobuf:"bytes,15,opt,name=runtime_config,json=runtimeConfig,proto3" json:"runtime_config,omitempty"`
	TheatreId          *string                    `protobuf:"bytes,16,opt,name=theatre_id,json=theatreId,proto3,oneof" json:"theatre_id,omitempty"`
	CapabilityId       *string                    `protobuf:"bytes,17,opt,name=capability_id,json=capabilityId,proto3,oneof" json:"capability_id,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *EvaluateDetectionRequest) Reset() {
	*x = EvaluateDetectionRequest{}
	mi := &file_mission_autonomy_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EvaluateDetectionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EvaluateDetectionRequest) ProtoMessage() {}

func (x *EvaluateDetectionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_mission_autonomy_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EvaluateDetectionRequest.ProtoReflect.Descriptor instead.
func (*EvaluateDetectionRequest) Descriptor() ([]byte, []int) {
	return file_mission_autonomy_proto_rawDescGZIP(), []int{0}
}

func (x *EvaluateDetectionRequest) GetBase() *proto.RequestBase {
	if x != nil {
		return x.Base
	}
	return nil
}

func (x *EvaluateDetectionRequest) GetDetectionId() string {
	if x != nil {
		return x.DetectionId
	}
	return ""
}

func (x *EvaluateDetectionRequest) GetAssetSn() string {
	if x != nil {
		return x.AssetSn
	}
	return ""
}

func (x *EvaluateDetectionRequest) GetObjectType() string {
	if x != nil {
		return x.ObjectType
	}
	return ""
}

func (x *EvaluateDetectionRequest) GetConfidence() float32 {
	if x != nil {
		return x.Confidence
	}
	return 0
}

func (x *EvaluateDetectionRequest) GetDetectionLatitude() float64 {
	if x != nil {
		return x.DetectionLatitude
	}
	return 0
}

func (x *EvaluateDetectionRequest) GetDetectionLongitude() float64 {
	if x != nil {
		return x.DetectionLongitude
	}
	return 0
}

func (x *EvaluateDetectionRequest) GetDetectionAltitude() float64 {
	if x != nil {
		return x.DetectionAltitude
	}
	return 0
}

func (x *EvaluateDetectionRequest) GetOrganizationId() string {
	if x != nil && x.OrganizationId != nil {
		return *x.OrganizationId
	}
	return ""
}

func (x *EvaluateDetectionRequest) GetRuntimeConfig() *proto1.DynamicConfigProto {
	if x != nil {
		return x.RuntimeConfig
	}
	return nil
}

func (x *EvaluateDetectionRequest) GetTheatreId() string {
	if x != nil && x.TheatreId != nil {
		return *x.TheatreId
	}
	return ""
}

func (x *EvaluateDetectionRequest) GetCapabilityId() string {
	if x != nil && x.CapabilityId != nil {
		return *x.CapabilityId
	}
	return ""
}

type DecisionResultProto struct {
	state              protoimpl.MessageState        `protogen:"open.v1"`
	DecisionId         string                        `protobuf:"bytes,1,opt,name=decision_id,json=decisionId,proto3" json:"decision_id,omitempty"`
	DetectionId        string                        `protobuf:"bytes,2,opt,name=detection_id,json=detectionId,proto3" json:"detection_id,omitempty"`
	SelectedAssetSn    *string                       `protobuf:"bytes,3,opt,name=selected_asset_sn,json=selectedAssetSn,proto3,oneof" json:"selected_asset_sn,omitempty"`
	StrategyUsed       *string                       `protobuf:"bytes,4,opt,name=strategy_used,json=strategyUsed,proto3,oneof" json:"strategy_used,omitempty"`
	Status             string                        `protobuf:"bytes,5,opt,name=status,proto3" json:"status,omitempty"` // DecisionStatus as string
	ConsideredAssetSns []string                      `protobuf:"bytes,6,rep,name=considered_asset_sns,json=consideredAssetSns,proto3" json:"considered_asset_sns,omitempty"`
	RejectionReasons   map[string]string             `protobuf:"bytes,7,rep,name=rejection_reasons,json=rejectionReasons,proto3" json:"rejection_reasons,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	DecidedAt          *timestamppb.Timestamp        `protobuf:"bytes,8,opt,name=decided_at,json=decidedAt,proto3" json:"decided_at,omitempty"`
	ResolvedConfig     *proto1.DynamicConfigProto    `protobuf:"bytes,12,opt,name=resolved_config,json=resolvedConfig,proto3" json:"resolved_config,omitempty"`
	SelectedActions    []*proto1.DecisionActionProto `protobuf:"bytes,13,rep,name=selected_actions,json=selectedActions,proto3" json:"selected_actions,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *DecisionResultProto) Reset() {
	*x = DecisionResultProto{}
	mi := &file_mission_autonomy_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DecisionResultProto) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DecisionResultProto) ProtoMessage() {}

func (x *DecisionResultProto) ProtoReflect() protoreflect.Message {
	mi := &file_mission_autonomy_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DecisionResultProto.ProtoReflect.Descriptor instead.
func (*DecisionResultProto) Descriptor() ([]byte, []int) {
	return file_mission_autonomy_proto_rawDescGZIP(), []int{1}
}

func (x *DecisionResultProto) GetDecisionId() string {
	if x != nil {
		return x.DecisionId
	}
	return ""
}

func (x *DecisionResultProto) GetDetectionId() string {
	if x != nil {
		return x.DetectionId
	}
	return ""
}

func (x *DecisionResultProto) GetSelectedAssetSn() string {
	if x != nil && x.SelectedAssetSn != nil {
		return *x.SelectedAssetSn
	}
	return ""
}

func (x *DecisionResultProto) GetStrategyUsed() string {
	if x != nil && x.StrategyUsed != nil {
		return *x.StrategyUsed
	}
	return ""
}

func (x *DecisionResultProto) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *DecisionResultProto) GetConsideredAssetSns() []string {
	if x != nil {
		return x.ConsideredAssetSns
	}
	return nil
}

func (x *DecisionResultProto) GetRejectionReasons() map[string]string {
	if x != nil {
		return x.RejectionReasons
	}
	return nil
}

func (x *DecisionResultProto) GetDecidedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.DecidedAt
	}
	return nil
}

func (x *DecisionResultProto) GetResolvedConfig() *proto1.DynamicConfigProto {
	if x != nil {
		return x.ResolvedConfig
	}
	return nil
}

func (x *DecisionResultProto) GetSelectedActions() []*proto1.DecisionActionProto {
	if x != nil {
		return x.SelectedActions
	}
	return nil
}

type DecisionResponse struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	HasErrors bool                   `protobuf:"varint,1,opt,name=has_errors,json=hasErrors,proto3" json:"has_errors,omitempty"`
	Tid       string                 `protobuf:"bytes,2,opt,name=tid,proto3" json:"tid,omitempty"`
	Timestamp *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=timestamp,proto3" json:"timestamp,omitempty"`
	// Types that are valid to be assigned to Response:
	//
	//	*DecisionResponse_Error
	//	*DecisionResponse_DecisionResult
	Response      isDecisionResponse_Response `protobuf_oneof:"response"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DecisionResponse) Reset() {
	*x = DecisionResponse{}
	mi := &file_mission_autonomy_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DecisionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DecisionResponse) ProtoMessage() {}

func (x *DecisionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_mission_autonomy_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DecisionResponse.ProtoReflect.Descriptor instead.
func (*DecisionResponse) Descriptor() ([]byte, []int) {
	return file_mission_autonomy_proto_rawDescGZIP(), []int{2}
}

func (x *DecisionResponse) GetHasErrors() bool {
	if x != nil {
		return x.HasErrors
	}
	return false
}

func (x *DecisionResponse) GetTid() string {
	if x != nil {
		return x.Tid
	}
	return ""
}

func (x *DecisionResponse) GetTimestamp() *timestamppb.Timestamp {
	if x != nil {
		return x.Timestamp
	}
	return nil
}

func (x *DecisionResponse) GetResponse() isDecisionResponse_Response {
	if x != nil {
		return x.Response
	}
	return nil
}

func (x *DecisionResponse) GetError() *proto.GlobalErrorMessage {
	if x != nil {
		if x, ok := x.Response.(*DecisionResponse_Error); ok {
			return x.Error
		}
	}
	return nil
}

func (x *DecisionResponse) GetDecisionResult() *DecisionResultProto {
	if x != nil {
		if x, ok := x.Response.(*DecisionResponse_DecisionResult); ok {
			return x.DecisionResult
		}
	}
	return nil
}

type isDecisionResponse_Response interface {
	isDecisionResponse_Response()
}

type DecisionResponse_Error struct {
	Error *proto.GlobalErrorMessage `protobuf:"bytes,4,opt,name=error,proto3,oneof"`
}

type DecisionResponse_DecisionResult struct {
	DecisionResult *DecisionResultProto `protobuf:"bytes,5,opt,name=decision_result,json=decisionResult,proto3,oneof"`
}

func (*DecisionResponse_Error) isDecisionResponse_Response() {}

func (*DecisionResponse_DecisionResult) isDecisionResponse_Response() {}

var File_mission_autonomy_proto protoreflect.FileDescriptor

const file_mission_autonomy_proto_rawDesc = "" +
	"\n" +
	"\x16mission-autonomy.proto\x12\x04zqnt\x1a\fcommon.proto\x1a$capability-execution-contracts.proto\x1a mission-autonomy-contracts.proto\x1a\x1fgoogle/protobuf/timestamp.proto\"\x8a\x05\n" +
	"\x18EvaluateDetectionRequest\x12%\n" +
	"\x04base\x18\x01 \x01(\v2\x11.zqnt.RequestBaseR\x04base\x12!\n" +
	"\fdetection_id\x18\x02 \x01(\tR\vdetectionId\x12\x19\n" +
	"\basset_sn\x18\x03 \x01(\tR\aassetSn\x12\x1f\n" +
	"\vobject_type\x18\x04 \x01(\tR\n" +
	"objectType\x12\x1e\n" +
	"\n" +
	"confidence\x18\x05 \x01(\x02R\n" +
	"confidence\x12-\n" +
	"\x12detection_latitude\x18\x06 \x01(\x01R\x11detectionLatitude\x12/\n" +
	"\x13detection_longitude\x18\a \x01(\x01R\x12detectionLongitude\x12-\n" +
	"\x12detection_altitude\x18\b \x01(\x01R\x11detectionAltitude\x12,\n" +
	"\x0forganization_id\x18\n" +
	" \x01(\tH\x00R\x0eorganizationId\x88\x01\x01\x12?\n" +
	"\x0eruntime_config\x18\x0f \x01(\v2\x18.zqnt.DynamicConfigProtoR\rruntimeConfig\x12\"\n" +
	"\n" +
	"theatre_id\x18\x10 \x01(\tH\x01R\ttheatreId\x88\x01\x01\x12(\n" +
	"\rcapability_id\x18\x11 \x01(\tH\x02R\fcapabilityId\x88\x01\x01B\x12\n" +
	"\x10_organization_idB\r\n" +
	"\v_theatre_idB\x10\n" +
	"\x0e_capability_idJ\x04\b\t\x10\n" +
	"J\x04\b\v\x10\fJ\x04\b\f\x10\rJ\x04\b\r\x10\x0eJ\x04\b\x0e\x10\x0fR\n" +
	"mission_idR\x0fmission_contextR\ftask_context\"\xc6\x05\n" +
	"\x13DecisionResultProto\x12\x1f\n" +
	"\vdecision_id\x18\x01 \x01(\tR\n" +
	"decisionId\x12!\n" +
	"\fdetection_id\x18\x02 \x01(\tR\vdetectionId\x12/\n" +
	"\x11selected_asset_sn\x18\x03 \x01(\tH\x00R\x0fselectedAssetSn\x88\x01\x01\x12(\n" +
	"\rstrategy_used\x18\x04 \x01(\tH\x01R\fstrategyUsed\x88\x01\x01\x12\x16\n" +
	"\x06status\x18\x05 \x01(\tR\x06status\x120\n" +
	"\x14considered_asset_sns\x18\x06 \x03(\tR\x12consideredAssetSns\x12\\\n" +
	"\x11rejection_reasons\x18\a \x03(\v2/.zqnt.DecisionResultProto.RejectionReasonsEntryR\x10rejectionReasons\x129\n" +
	"\n" +
	"decided_at\x18\b \x01(\v2\x1a.google.protobuf.TimestampR\tdecidedAt\x12A\n" +
	"\x0fresolved_config\x18\f \x01(\v2\x18.zqnt.DynamicConfigProtoR\x0eresolvedConfig\x12D\n" +
	"\x10selected_actions\x18\r \x03(\v2\x19.zqnt.DecisionActionProtoR\x0fselectedActions\x1aC\n" +
	"\x15RejectionReasonsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01B\x14\n" +
	"\x12_selected_asset_snB\x10\n" +
	"\x0e_strategy_usedJ\x04\b\t\x10\n" +
	"J\x04\b\n" +
	"\x10\vJ\x04\b\v\x10\fR\x13selected_mission_idR\x10selected_task_id\"\x81\x02\n" +
	"\x10DecisionResponse\x12\x1d\n" +
	"\n" +
	"has_errors\x18\x01 \x01(\bR\thasErrors\x12\x10\n" +
	"\x03tid\x18\x02 \x01(\tR\x03tid\x128\n" +
	"\ttimestamp\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\ttimestamp\x120\n" +
	"\x05error\x18\x04 \x01(\v2\x18.zqnt.GlobalErrorMessageH\x00R\x05error\x12D\n" +
	"\x0fdecision_result\x18\x05 \x01(\v2\x19.zqnt.DecisionResultProtoH\x00R\x0edecisionResultB\n" +
	"\n" +
	"\bresponse2\xf9\x0f\n" +
	"\x16MissionAutonomyService\x12N\n" +
	"\x11UpsertApplication\x12\x1e.zqnt.UpsertApplicationRequest\x1a\x19.zqnt.ApplicationResponse\x12H\n" +
	"\x0eGetApplication\x12\x1b.zqnt.GetApplicationRequest\x1a\x19.zqnt.ApplicationResponse\x12P\n" +
	"\x10ListApplications\x12\x1d.zqnt.ListApplicationsRequest\x1a\x1d.zqnt.ApplicationListResponse\x12N\n" +
	"\x11DeleteApplication\x12\x1e.zqnt.DeleteApplicationRequest\x1a\x19.zqnt.ApplicationResponse\x12l\n" +
	"\x1aGetApplicationEnvironments\x12'.zqnt.GetApplicationEnvironmentsRequest\x1a%.zqnt.ApplicationEnvironmentsResponse\x12j\n" +
	"\x19PromoteApplicationVersion\x12&.zqnt.PromoteApplicationVersionRequest\x1a%.zqnt.ApplicationEnvironmentsResponse\x12W\n" +
	"\x14CreateSkillExecution\x12!.zqnt.CreateSkillExecutionRequest\x1a\x1c.zqnt.SkillExecutionResponse\x12G\n" +
	"\fExecuteSkill\x12\x19.zqnt.ExecuteSkillRequest\x1a\x1c.zqnt.SkillExecutionResponse\x12Q\n" +
	"\x11GetSkillExecution\x12\x1e.zqnt.GetSkillExecutionRequest\x1a\x1c.zqnt.SkillExecutionResponse\x12Y\n" +
	"\x13ListSkillExecutions\x12 .zqnt.ListSkillExecutionsRequest\x1a .zqnt.SkillExecutionListResponse\x12Y\n" +
	"\x13StartSkillExecution\x12$.zqnt.SkillExecutionLifecycleRequest\x1a\x1c.zqnt.SkillExecutionResponse\x12Y\n" +
	"\x13PauseSkillExecution\x12$.zqnt.SkillExecutionLifecycleRequest\x1a\x1c.zqnt.SkillExecutionResponse\x12Z\n" +
	"\x14ResumeSkillExecution\x12$.zqnt.SkillExecutionLifecycleRequest\x1a\x1c.zqnt.SkillExecutionResponse\x12Z\n" +
	"\x14CancelSkillExecution\x12$.zqnt.SkillExecutionLifecycleRequest\x1a\x1c.zqnt.SkillExecutionResponse\x12W\n" +
	"\x14SignalSkillExecution\x12!.zqnt.SignalSkillExecutionRequest\x1a\x1c.zqnt.SkillExecutionResponse\x12c\n" +
	"\x16ResolveExecutionConfig\x12#.zqnt.ResolveExecutionConfigRequest\x1a$.zqnt.ResolveExecutionConfigResponse\x12F\n" +
	"\x0eListSchedulers\x12\x1b.zqnt.ListSchedulersRequest\x1a\x17.zqnt.SchedulerResponse\x12B\n" +
	"\fGetScheduler\x12\x19.zqnt.GetSchedulerRequest\x1a\x17.zqnt.SchedulerResponse\x12H\n" +
	"\x0fCreateScheduler\x12\x1c.zqnt.CreateSchedulerRequest\x1a\x17.zqnt.SchedulerResponse\x12H\n" +
	"\x0fUpdateScheduler\x12\x1c.zqnt.UpdateSchedulerRequest\x1a\x17.zqnt.SchedulerResponse\x12H\n" +
	"\x0fDeleteScheduler\x12\x1c.zqnt.DeleteSchedulerRequest\x1a\x17.zqnt.SchedulerResponse\x12J\n" +
	"\x10CreateSchedulers\x12\x1d.zqnt.CreateSchedulersRequest\x1a\x17.zqnt.SchedulerResponse\x12J\n" +
	"\x10DeleteSchedulers\x12\x1d.zqnt.DeleteSchedulersRequest\x1a\x17.zqnt.SchedulerResponse\x12K\n" +
	"\x11EvaluateDetection\x12\x1e.zqnt.EvaluateDetectionRequest\x1a\x16.zqnt.DecisionResponseBQ\n" +
	"\x1ccom.zqnt.utils.mission.protoB\x14MissionAutonomyProtoP\x01Z\x19gen/missionautonomy/protob\x06proto3"

var (
	file_mission_autonomy_proto_rawDescOnce sync.Once
	file_mission_autonomy_proto_rawDescData []byte
)

func file_mission_autonomy_proto_rawDescGZIP() []byte {
	file_mission_autonomy_proto_rawDescOnce.Do(func() {
		file_mission_autonomy_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_mission_autonomy_proto_rawDesc), len(file_mission_autonomy_proto_rawDesc)))
	})
	return file_mission_autonomy_proto_rawDescData
}

var file_mission_autonomy_proto_msgTypes = make([]protoimpl.MessageInfo, 4)
var file_mission_autonomy_proto_goTypes = []any{
	(*EvaluateDetectionRequest)(nil),                 // 0: zqnt.EvaluateDetectionRequest
	(*DecisionResultProto)(nil),                      // 1: zqnt.DecisionResultProto
	(*DecisionResponse)(nil),                         // 2: zqnt.DecisionResponse
	nil,                                              // 3: zqnt.DecisionResultProto.RejectionReasonsEntry
	(*proto.RequestBase)(nil),                        // 4: zqnt.RequestBase
	(*proto1.DynamicConfigProto)(nil),                // 5: zqnt.DynamicConfigProto
	(*timestamppb.Timestamp)(nil),                    // 6: google.protobuf.Timestamp
	(*proto1.DecisionActionProto)(nil),               // 7: zqnt.DecisionActionProto
	(*proto.GlobalErrorMessage)(nil),                 // 8: zqnt.GlobalErrorMessage
	(*proto2.UpsertApplicationRequest)(nil),          // 9: zqnt.UpsertApplicationRequest
	(*proto2.GetApplicationRequest)(nil),             // 10: zqnt.GetApplicationRequest
	(*proto2.ListApplicationsRequest)(nil),           // 11: zqnt.ListApplicationsRequest
	(*proto2.DeleteApplicationRequest)(nil),          // 12: zqnt.DeleteApplicationRequest
	(*proto2.GetApplicationEnvironmentsRequest)(nil), // 13: zqnt.GetApplicationEnvironmentsRequest
	(*proto2.PromoteApplicationVersionRequest)(nil),  // 14: zqnt.PromoteApplicationVersionRequest
	(*proto2.CreateSkillExecutionRequest)(nil),       // 15: zqnt.CreateSkillExecutionRequest
	(*proto2.ExecuteSkillRequest)(nil),               // 16: zqnt.ExecuteSkillRequest
	(*proto2.GetSkillExecutionRequest)(nil),          // 17: zqnt.GetSkillExecutionRequest
	(*proto2.ListSkillExecutionsRequest)(nil),        // 18: zqnt.ListSkillExecutionsRequest
	(*proto2.SkillExecutionLifecycleRequest)(nil),    // 19: zqnt.SkillExecutionLifecycleRequest
	(*proto2.SignalSkillExecutionRequest)(nil),       // 20: zqnt.SignalSkillExecutionRequest
	(*proto2.ResolveExecutionConfigRequest)(nil),     // 21: zqnt.ResolveExecutionConfigRequest
	(*proto3.ListSchedulersRequest)(nil),             // 22: zqnt.ListSchedulersRequest
	(*proto3.GetSchedulerRequest)(nil),               // 23: zqnt.GetSchedulerRequest
	(*proto3.CreateSchedulerRequest)(nil),            // 24: zqnt.CreateSchedulerRequest
	(*proto3.UpdateSchedulerRequest)(nil),            // 25: zqnt.UpdateSchedulerRequest
	(*proto3.DeleteSchedulerRequest)(nil),            // 26: zqnt.DeleteSchedulerRequest
	(*proto3.CreateSchedulersRequest)(nil),           // 27: zqnt.CreateSchedulersRequest
	(*proto3.DeleteSchedulersRequest)(nil),           // 28: zqnt.DeleteSchedulersRequest
	(*proto2.ApplicationResponse)(nil),               // 29: zqnt.ApplicationResponse
	(*proto2.ApplicationListResponse)(nil),           // 30: zqnt.ApplicationListResponse
	(*proto2.ApplicationEnvironmentsResponse)(nil),   // 31: zqnt.ApplicationEnvironmentsResponse
	(*proto2.SkillExecutionResponse)(nil),            // 32: zqnt.SkillExecutionResponse
	(*proto2.SkillExecutionListResponse)(nil),        // 33: zqnt.SkillExecutionListResponse
	(*proto2.ResolveExecutionConfigResponse)(nil),    // 34: zqnt.ResolveExecutionConfigResponse
	(*proto3.SchedulerResponse)(nil),                 // 35: zqnt.SchedulerResponse
}
var file_mission_autonomy_proto_depIdxs = []int32{
	4,  // 0: zqnt.EvaluateDetectionRequest.base:type_name -> zqnt.RequestBase
	5,  // 1: zqnt.EvaluateDetectionRequest.runtime_config:type_name -> zqnt.DynamicConfigProto
	3,  // 2: zqnt.DecisionResultProto.rejection_reasons:type_name -> zqnt.DecisionResultProto.RejectionReasonsEntry
	6,  // 3: zqnt.DecisionResultProto.decided_at:type_name -> google.protobuf.Timestamp
	5,  // 4: zqnt.DecisionResultProto.resolved_config:type_name -> zqnt.DynamicConfigProto
	7,  // 5: zqnt.DecisionResultProto.selected_actions:type_name -> zqnt.DecisionActionProto
	6,  // 6: zqnt.DecisionResponse.timestamp:type_name -> google.protobuf.Timestamp
	8,  // 7: zqnt.DecisionResponse.error:type_name -> zqnt.GlobalErrorMessage
	1,  // 8: zqnt.DecisionResponse.decision_result:type_name -> zqnt.DecisionResultProto
	9,  // 9: zqnt.MissionAutonomyService.UpsertApplication:input_type -> zqnt.UpsertApplicationRequest
	10, // 10: zqnt.MissionAutonomyService.GetApplication:input_type -> zqnt.GetApplicationRequest
	11, // 11: zqnt.MissionAutonomyService.ListApplications:input_type -> zqnt.ListApplicationsRequest
	12, // 12: zqnt.MissionAutonomyService.DeleteApplication:input_type -> zqnt.DeleteApplicationRequest
	13, // 13: zqnt.MissionAutonomyService.GetApplicationEnvironments:input_type -> zqnt.GetApplicationEnvironmentsRequest
	14, // 14: zqnt.MissionAutonomyService.PromoteApplicationVersion:input_type -> zqnt.PromoteApplicationVersionRequest
	15, // 15: zqnt.MissionAutonomyService.CreateSkillExecution:input_type -> zqnt.CreateSkillExecutionRequest
	16, // 16: zqnt.MissionAutonomyService.ExecuteSkill:input_type -> zqnt.ExecuteSkillRequest
	17, // 17: zqnt.MissionAutonomyService.GetSkillExecution:input_type -> zqnt.GetSkillExecutionRequest
	18, // 18: zqnt.MissionAutonomyService.ListSkillExecutions:input_type -> zqnt.ListSkillExecutionsRequest
	19, // 19: zqnt.MissionAutonomyService.StartSkillExecution:input_type -> zqnt.SkillExecutionLifecycleRequest
	19, // 20: zqnt.MissionAutonomyService.PauseSkillExecution:input_type -> zqnt.SkillExecutionLifecycleRequest
	19, // 21: zqnt.MissionAutonomyService.ResumeSkillExecution:input_type -> zqnt.SkillExecutionLifecycleRequest
	19, // 22: zqnt.MissionAutonomyService.CancelSkillExecution:input_type -> zqnt.SkillExecutionLifecycleRequest
	20, // 23: zqnt.MissionAutonomyService.SignalSkillExecution:input_type -> zqnt.SignalSkillExecutionRequest
	21, // 24: zqnt.MissionAutonomyService.ResolveExecutionConfig:input_type -> zqnt.ResolveExecutionConfigRequest
	22, // 25: zqnt.MissionAutonomyService.ListSchedulers:input_type -> zqnt.ListSchedulersRequest
	23, // 26: zqnt.MissionAutonomyService.GetScheduler:input_type -> zqnt.GetSchedulerRequest
	24, // 27: zqnt.MissionAutonomyService.CreateScheduler:input_type -> zqnt.CreateSchedulerRequest
	25, // 28: zqnt.MissionAutonomyService.UpdateScheduler:input_type -> zqnt.UpdateSchedulerRequest
	26, // 29: zqnt.MissionAutonomyService.DeleteScheduler:input_type -> zqnt.DeleteSchedulerRequest
	27, // 30: zqnt.MissionAutonomyService.CreateSchedulers:input_type -> zqnt.CreateSchedulersRequest
	28, // 31: zqnt.MissionAutonomyService.DeleteSchedulers:input_type -> zqnt.DeleteSchedulersRequest
	0,  // 32: zqnt.MissionAutonomyService.EvaluateDetection:input_type -> zqnt.EvaluateDetectionRequest
	29, // 33: zqnt.MissionAutonomyService.UpsertApplication:output_type -> zqnt.ApplicationResponse
	29, // 34: zqnt.MissionAutonomyService.GetApplication:output_type -> zqnt.ApplicationResponse
	30, // 35: zqnt.MissionAutonomyService.ListApplications:output_type -> zqnt.ApplicationListResponse
	29, // 36: zqnt.MissionAutonomyService.DeleteApplication:output_type -> zqnt.ApplicationResponse
	31, // 37: zqnt.MissionAutonomyService.GetApplicationEnvironments:output_type -> zqnt.ApplicationEnvironmentsResponse
	31, // 38: zqnt.MissionAutonomyService.PromoteApplicationVersion:output_type -> zqnt.ApplicationEnvironmentsResponse
	32, // 39: zqnt.MissionAutonomyService.CreateSkillExecution:output_type -> zqnt.SkillExecutionResponse
	32, // 40: zqnt.MissionAutonomyService.ExecuteSkill:output_type -> zqnt.SkillExecutionResponse
	32, // 41: zqnt.MissionAutonomyService.GetSkillExecution:output_type -> zqnt.SkillExecutionResponse
	33, // 42: zqnt.MissionAutonomyService.ListSkillExecutions:output_type -> zqnt.SkillExecutionListResponse
	32, // 43: zqnt.MissionAutonomyService.StartSkillExecution:output_type -> zqnt.SkillExecutionResponse
	32, // 44: zqnt.MissionAutonomyService.PauseSkillExecution:output_type -> zqnt.SkillExecutionResponse
	32, // 45: zqnt.MissionAutonomyService.ResumeSkillExecution:output_type -> zqnt.SkillExecutionResponse
	32, // 46: zqnt.MissionAutonomyService.CancelSkillExecution:output_type -> zqnt.SkillExecutionResponse
	32, // 47: zqnt.MissionAutonomyService.SignalSkillExecution:output_type -> zqnt.SkillExecutionResponse
	34, // 48: zqnt.MissionAutonomyService.ResolveExecutionConfig:output_type -> zqnt.ResolveExecutionConfigResponse
	35, // 49: zqnt.MissionAutonomyService.ListSchedulers:output_type -> zqnt.SchedulerResponse
	35, // 50: zqnt.MissionAutonomyService.GetScheduler:output_type -> zqnt.SchedulerResponse
	35, // 51: zqnt.MissionAutonomyService.CreateScheduler:output_type -> zqnt.SchedulerResponse
	35, // 52: zqnt.MissionAutonomyService.UpdateScheduler:output_type -> zqnt.SchedulerResponse
	35, // 53: zqnt.MissionAutonomyService.DeleteScheduler:output_type -> zqnt.SchedulerResponse
	35, // 54: zqnt.MissionAutonomyService.CreateSchedulers:output_type -> zqnt.SchedulerResponse
	35, // 55: zqnt.MissionAutonomyService.DeleteSchedulers:output_type -> zqnt.SchedulerResponse
	2,  // 56: zqnt.MissionAutonomyService.EvaluateDetection:output_type -> zqnt.DecisionResponse
	33, // [33:57] is the sub-list for method output_type
	9,  // [9:33] is the sub-list for method input_type
	9,  // [9:9] is the sub-list for extension type_name
	9,  // [9:9] is the sub-list for extension extendee
	0,  // [0:9] is the sub-list for field type_name
}

func init() { file_mission_autonomy_proto_init() }
func file_mission_autonomy_proto_init() {
	if File_mission_autonomy_proto != nil {
		return
	}
	file_mission_autonomy_proto_msgTypes[0].OneofWrappers = []any{}
	file_mission_autonomy_proto_msgTypes[1].OneofWrappers = []any{}
	file_mission_autonomy_proto_msgTypes[2].OneofWrappers = []any{
		(*DecisionResponse_Error)(nil),
		(*DecisionResponse_DecisionResult)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_mission_autonomy_proto_rawDesc), len(file_mission_autonomy_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   4,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_mission_autonomy_proto_goTypes,
		DependencyIndexes: file_mission_autonomy_proto_depIdxs,
		MessageInfos:      file_mission_autonomy_proto_msgTypes,
	}.Build()
	File_mission_autonomy_proto = out.File
	file_mission_autonomy_proto_goTypes = nil
	file_mission_autonomy_proto_depIdxs = nil
}
